---
title: "Unf*ck Your Sales"
author: "Jakob Greenfeld"
rating: 8
slug: "unfck-your-sales"
description: "A practical framework for B2B sales built around four constraints—reach, resonance, timing, and trust—using inversion and the Theory of Constraints to find and fix the single bottleneck in your pipeline."
cover: "./images/unfck-your-sales.jpg"
dateRead: 2025-10-26
dateCreated: 2026-01-10
dateUpdated: 2026-01-10
category: "Business"
type: "book"
notAffiliateLink: https://jakobgreenfeld.gumroad.com/l/sales
hasSummaries: false
tags:
  - Sales
  - Business
...
1) "Companies buy when you have the attention of the right people, at the right time, and they trust you enough to take action."
...
2) "Every system is limited by one key constraint."
...
3) "Until you’ve deliberately and directly put your name in front of everyone in your actual market, at least once, you can’t claim rejection."
...
4) "Your message has to answer three questions instantly: What painful problem do you solve? What outcome do I get? How do you get me there?"
...
5) "B2B buyers don’t trust confidence. They trust evidence."
...
#### Supporting Problems
1. Why do teams try to improve conversion, persuasion, positioning, or funnels before they’ve earned basic attention from the right buyers?
2. How do you define a “market” as a finite, reachable list of real companies and specific buyers (a hit list), rather than an abstract TAM?
3. What are the most common failure modes in reach (over-precision/“starving” vs over-volume/“bingeing”), and how do you avoid both?
4. How do you discover the minimally sufficient “non-negotiable” criteria for targeting based on proof from closed deals (yours or competitors’)?
5. Which outreach channels actually provide controlled, provable delivery to qualified buyers (vs brand-only or hope-based channels)?
6. How do you know whether your message resonates—i.e., buyers can quickly recall who you are, what you do, and the pain/outcome you own?
7. Why does messaging fail due to vagueness, abstraction, and complexity, and how do you make it blunt, repeatable, and buyer-native?
8. How should a message be structured so a buyer gets it in seconds (pain → concrete outcome → mechanism)?
9. Why do deals get lost before you’re even considered (shortlists), and how do you become distinctive and memorable rather than “unique”?
10. Why is timing not something you “detect” via intent, but something you engineer through consistent presence and repetition?
11. How do you overcome the fear of “annoying” prospects and replace it with a cadence that builds memory (e.g., reminders every 4–6 weeks)?
12. Why is a low close rate on qualified leads usually a trust constraint, and how do you diagnose it (ghosting, “sounds good” then vanish)?
13. What builds B2B trust (evidence and risk reduction) versus what destroys it (confidence, hype, loud pitching without proof)?
14. How do buyers actually make decisions (safe > best; minimizing regret), and how should sales adapt to that?
15. What does “enough trust” statistically require (multi-hour, multi-touchpoint, multi-platform exposure), and how do you design for it?
16. How do you reduce friction that blocks trust and action (simple offer, sharp pitch, clear pricing, obvious next steps)?
17. How do you build “digital gravity” (ongoing mindshare with the 95% not ready now) rather than obsessing over funnel optimization?
...
> **Prompt skeleton:**
> You are my sales ops analyst using *Theory of Constraints* and *inversion*.
> Context: [briefly describe product, price, market].
> Data:
> - Stages + conversion rates: [paste]
> - Volume by stage: [paste]
> - Last 20 lost/ghosted reasons: [paste]
> Task:
> 1) Identify the single biggest constraint limiting revenue (choose only one): Attention, Resonance, Timing, or Trust.
> 2) Show your reasoning using the data above.
> 3) Propose 3 fixes ranked by leverage and speed.
> 4) Create a 14-day plan with daily actions and a simple success metric for each day.
> Output in a table.
...
> **Prompt skeleton:**
> Act as a B2B revenue analyst. I will paste closed-won deal data.
> Data fields: [list fields].
> Closed-won deals: [paste rows].
> Task:
> 1) Infer the *non-negotiable* criteria that appear necessary for a deal to close (aim for 3–7).
> 2) Separate “nice-to-have” from “must-have.”
> 3) Write the criteria as filter rules I could apply in a spreadsheet/CRM.
> 4) Provide an exclusion list: patterns that correlate with churn, long cycles, or no-decision.
> Format: bullets + a final criteria checklist.
...
> **Prompt skeleton:**
> You are my ABM list-building assistant.
> Criteria (non-negotiables): [paste].
> Source data (companies/leads): [paste].
> Buyer roles that feel pain + can buy: [paste].
> Task:
> 1) Select only accounts that meet all non-negotiables.
> 2) For each account, identify 1–3 likely buyer roles and the order to contact them.
> 3) Add a confidence score (High/Med/Low) and why.
> Output as a table with columns: Account | Why fit | Buyer #1 role | Buyer #2 role | Buyer #3 role | Confidence | Notes.
...
> **Prompt skeleton:**
> You are my outbound operations planner.
> Hit list size: [N accounts / N contacts].
> Available channels (only use these): [list].
> Time budget: [hours/week]. Tools: [tools]. Constraints: [GDPR/industry].
> Task: Build a 4-week plan to ensure every buyer sees us at least once (“saturate before optimize”).
> Include: daily volumes, sequencing across channels, and what I should track.
> Output: a weekly calendar + a tracking template.
...
> **Prompt skeleton:**
> Act as a conversion-focused messaging editor.
> Buyer pain (use their words): “[paste]”
> Outcome (concrete): [paste]
> Mechanism (what we do): [paste]
> Proof snippets (optional): [paste]
> Task:
> 1) Write ONE core sentence combining Pain + Outcome + Mechanism.
> 2) Create 5 channel variations that keep the meaning identical (email subject+first line, DM, phone opener, website hero, ad headline).
> 3) List 10 vague/fluffy terms to avoid in our messaging.
> Keep everything blunt, simple, and specific.
...
> **Prompt skeleton:**
> You are my research assistant running a mindshare check.
> Category/problem phrase: [e.g., “reducing employee churn in retail”].
> Audience description: [who they are].
> Current message: [paste].
> Task:
> 1) Draft a 3-question micro-survey whose first question is: “When you think of [category], who comes to mind?”
> 2) Draft the outreach message to invite 50 people (offer incentive).
> 3) Provide an analysis framework: how to code responses, what “pass/fail” looks like, and what to change if we’re not mentioned.
> Output: survey + outreach + analysis rubric.
...
> **Prompt skeleton:**
> You are my lifecycle touchpoint designer.
> Audience segments: [list segments + pain].
> Channels I can use: [list].
> Assets: [case studies, posts, Looms, docs].
> Rules: remind, don’t pitch; frequency cap: [e.g., max 1 message/week].
> Task: Create a 6-week reminder plan so each buyer gets 1–2 lightweight touches every 4–6 weeks.
> For each touch: objective, message template, and the asset to include.
> Output as a table by week and segment.
...
> **Prompt skeleton:**
> Act as my trust architect for B2B buyers who optimize for safety.
> My claims: [paste].
> Objections/risks buyers mention: [paste].
> Existing proof assets: [paste links/snippets].
> Task:
> 1) Build a matrix mapping each claim to the buyer risk it triggers and the best evidence to reduce that risk.
> 2) Identify proof gaps and propose the fastest proof to create (screenshots, mini case study, benchmark, etc.).
> 3) Write 10 “proof snippets” (1–2 sentences) I can paste into DMs/emails.
> Output: matrix + gaps + snippets.
...
> **Prompt skeleton:**
> You are my multi-touch journey designer.
> Platforms I can use: [e.g., Email, LinkedIn, YouTube, Website].
> Current assets: [list with length].
> Weekly capacity: [hours].
> Task:
> 1) Create a sequence of 11 touchpoints across at least 4 platforms totaling ~7 hours of exposure.
> 2) Each touchpoint must have: purpose (remind/prove/teach), asset, expected time spent, and CTA (if any).
> 3) Provide a minimal logging template for CRM.
> Output as a numbered list + logging template.
...
> **Prompt skeleton:**
> You are my deal-rescue assistant. Use inversion: assume I’m failing due to attention/timing/trust/mismatch and find the most plausible one.
> Deal context: [paste].
> Messages so far: [paste thread].
> Buyer + role: [paste].
> Task:
> 1) Diagnose the most likely reason for silence and rate confidence.
> 2) Write 3 follow-ups (<=80 words) using: (a) proof, (b) clarity on next step, (c) timing check. No guilt, no pressure.
> 3) Write 1 short breakup message.
...
> **Prompt skeleton:**
> Act as a ruthless clarity editor for B2B buyers who avoid regret.
> Materials:
> - Offer/landing copy: [paste]
> - Pricing: [paste]
> - Next steps: [paste]
> Task:
> 1) Identify points of friction or vagueness that reduce trust (rank by severity).
> 2) Rewrite the key sections to be dead simple (no buzzwords).
> 3) Produce a 3-step “what happens next” script I can say on calls and paste in follow-up emails.
> Output: ranked list + rewrites + script.
...
> **Prompt skeleton:**
> You are my consistency editor. My goal is distinctiveness through repetition, not novelty.
> Core message (must remain consistent): [paste].
> Templates/content to review: [paste].
> Task:
> 1) Create a rubric to check if a message preserves the same Pain/Outcome/Mechanism (pass/fail).
> 2) Edit each template to align with the core message while staying natural for the channel.
> 3) Output reusable blocks: Pain lines, Outcome lines, Mechanism lines, Proof lines (5 each).
